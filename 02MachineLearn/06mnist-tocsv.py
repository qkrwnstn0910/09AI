...
def to_csv(name, maxdata):
    ...
    # 변환된 데이터를 저장할 csv파일 경로 (쓰기모드)
    csv_f = open("./resMnist/"+name+".csv","w", encoding="utf-8")

    '''struct 모듈을 사용하여 바이너리 데이터를 읽고 정수로 변환한다. >II는
    빅엔디안 방식으로 4바이트 정수 2개를 읽겠다는 의미로 사용된다.'''
    ...
    for idx in range(lbl_count):
        ...
        # 잘 저장되었는지 눈으로 확인하기 위해 일부 이미지를 PGM포맷으로 저장
        if idx < 10: #처음 10개만 저장한다.
            s= "P2 28 28 255\n" #pGM 헤더(ASCII 형싱의 그레이스 케일 이미지)
            s += " ".join(sdata) #픽셀데이터를 공백으로 구분하여 추가
            # 저장된 파일의 경로 지정
