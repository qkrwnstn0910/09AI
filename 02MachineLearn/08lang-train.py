...
def check_freq(fname):
    ...
    '''
    파일명은 en-1.txt와 같은 형식을 가지고 있다. 정규표현식을 통해 파일명
    앞부분의 단어를 얻어온다.
        ^:문자열의 시작을 의미
        [a~z]:소문자 a부터 z중 하나를 의미
        {2,}:2개이상 반복됨을 의미
    group()은 매치된 부분의 문자열을 반환한다.
    즉 en, fr등의 문자열이 lang에 저장된다.'''
...
def load_files(path):
    ...
    freqs = []
    labels = []
    #*.txt와 같은 패턴을 이용해서 파일의 목록을 리스트로 생성
...
print("학습 데이터 개수:", len(data["freqs"]))
print("테스트 데이터 개수:", len(test["freqs"]))
